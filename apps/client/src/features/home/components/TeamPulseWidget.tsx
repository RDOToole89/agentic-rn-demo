...
import { StatusDot } from '../../pulse/components/StatusDot';
import { MoodDistribution } from '../../pulse/components/MoodDistribution';
...
export function TeamPulseWidget({ members, onViewPulse }: TeamPulseWidgetProps) {
  ...
  return (
    <View className="p-5 rounded-2xl bg-card shadow-sm border-l-4 border-accent">
      {/* Header row: vibe + view link */}
      <View className="flex-row items-center justify-between mb-4">
        <View className="flex-row items-center gap-3">
          <Text className="text-4xl">{vibe.emoji}</Text>
          <View>
            <Text className="text-xs text-text-secondary">Team Vibe</Text>
            <Text className="text-xl font-bold text-text-primary">{vibe.label}</Text>
          </View>
        </View>
        <Pressable onPress={onViewPulse}>
          <Text className="text-sm font-semibold text-accent">View Pulse →</Text>
        </Pressable>
      </View>
